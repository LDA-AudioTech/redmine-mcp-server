...
async def attach_file_to_issue(
    ...
    **Content sources — provide exactly ONE of:**
    ...
    update_fields: Dict[str, Any] = {"uploads": [upload_entry]}
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"Upload succeeded (token={token}) but failed to attach to issue {issue_id}: {_scrub_error_message(str(e))}",
            "token": token,
            "filename": upload_filename,
            "content_type": upload_content_type,
            "size": upload_result.get("size"),
            "hint": "You can retry the association manually with update_redmine_issue using the 'uploads' field and this token.",
        }

    try:
        updated_issue = client.issue.get(issue_id)
        attachments = _attachments_to_list(updated_issue)
        uploaded = None
